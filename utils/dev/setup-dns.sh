...
DNS_CONF="$DNS_OUTPUT_DIR/dkhfst-dd.${PROJECT_NAME}.conf"
TMP_CONF="$DNS_CONF.tmp"

echo "🔧 Generating DNS config..."
python3 -m utils.dev.helpers.setup_dns > "$TMP_CONF"
...
SYMLINK_TARGET="$DNSMASQ_DIR/dkhfst-dd.${PROJECT_NAME}.conf"
